  ...
  <q-dialog v-model="show" maximized transition-show="slide-up" transition-hide="slide-down">
    <div class="post-detail-dialog q-pa-md">
      <div class="flex justify-between items-center q-mb-lg">
        <span class="text-white text-subtitle1 text-weight-semibold">{{ $t('detail.title') }}</span>
        <q-btn flat round dense icon="close" color="white" @click="show = false" />
      </div>
      ...
      <template v-else-if="post">
        ...
        <q-item class="q-pa-none q-mb-md">
          <q-item-section avatar>
            <q-avatar color="primary" text-color="white">
              ...
              />
              <span v-else>{{ post.user?.identification?.[0]?.toUpperCase() || '?' }}</span>
            </q-avatar>
          </q-item-section>
          <q-item-section>
            <q-item-label style="font-size: 0.9rem; line-height: 1.3">
              <span class="text-white text-weight-semibold">{{ post.user?.userName }}</span>
              <span style="color: rgba(150,170,220,0.5); font-size: 0.8rem"> @{{ post.user?.identification }}</span>
              <span style="color: rgba(150,170,220,0.35); font-size: 0.8rem"> · {{ formatPostDate(post.createdAt) }}</span>
            </q-item-label>
          </q-item-section>
        ...
        <p
          class="q-mb-lg"
          style="white-space: pre-wrap; word-break: break-word; color: #dde4f0; line-height: 1.55; font-size: 0.95rem"
        >
          {{ post.content }}
        </p>
        ...
        <q-separator style="background: rgba(79,134,247,0.12)" class="q-mb-md" />
...
function syncFromPost() {
  if (post.value) {
    ...
    commentCount.value = Array.isArray(post.value.comments) ? post.value.comments.length : 0
  }
}
