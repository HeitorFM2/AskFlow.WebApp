...
export const useAuthStore = defineStore('auth', () => {
  ...
  async function register(email, password, userName, identification) {
    const data = await AuthService.register({ email, password, userName, identification })
    _persist(data)
    return data
  }
  ...
  return { user, accessToken, isAuthenticated, login, register, logout, updateAvatar }
})
