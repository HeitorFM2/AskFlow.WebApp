...
export const useCommentsStore = defineStore('comments', () => {
  ...
  async function fetchReplies(commentId, page = 1, pageSize = 20) {
    loading.value = true
    try {
      const result = await CommentsService.getReplies(commentId, page, pageSize)
  ...
  async function addReply(postId, commentId, content) {
    await CommentsService.create(postId, { content, parentCommentId: commentId })
  ...
  async function removeComment(postId, commentId) {
    ...
    if (byPost.value[postId]) {
      byPost.value[postId].items = byPost.value[postId].items.filter((c) => c.id !== commentId)
    }
  }

  function getPostComments(postId) {
    return byPost.value[postId] || { items: [], totalCount: 0, hasNextPage: false }
  }
